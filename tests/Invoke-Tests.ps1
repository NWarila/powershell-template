...
<#
.SYNOPSIS
    Runs the SampleModule Pester v5 suite with NUnit output and code coverage.

.DESCRIPTION
    Builds a PesterConfiguration object explicitly (rather than relying on
    Invoke-Pester parameter splatting) so the same configuration is used
    locally and in CI. Emits an NUnit XML result file and a JaCoCo coverage
    report under the output directory, and enforces a minimum coverage target.

    Run locally with: pwsh -File tests/Invoke-Tests.ps1
...
$config.CodeCoverage.Path = Join-Path -Path $repoRoot -ChildPath 'src'
