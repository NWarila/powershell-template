...
<#
.SYNOPSIS
    Builds, analyzes, tests, and smokes the current PowerShell repo layout.

.PARAMETER Task
    One or more tasks to run: Build, Test, Analyze, Smoke, Clean, or All.

.PARAMETER MinimumCoverage
    Minimum line-coverage percentage required for Test. The default sentinel
    selects the layout default: 90 for script repos, 80 for module repos.
...
[System.Double]$Script:RequestedMinimumCoverage = $MinimumCoverage
...
Function Get-RepoLayout {
  ...
  [System.Boolean]$Private:HasScript = Test-Path -LiteralPath (Join-Path -Path $ProjectRoot -ChildPath '.github/.script')
  [System.Boolean]$Private:HasModule = Test-Path -LiteralPath (Join-Path -Path $ProjectRoot -ChildPath '.github/.module')

  If ($HasScript -eq $HasModule) {
    Throw 'Repo layout marker invalid: exactly one of .github/.script or .github/.module must exist.'
  }
...
Function Get-ModuleManifestPath {
  ...
  [System.IO.FileInfo[]]$Private:ManifestFiles = @(
    Get-ChildItem `
  ...
  If ($ManifestFiles.Count -ne 1) {
    Throw ('Module layout requires exactly one manifest under src; found {0}.' -f $ManifestFiles.Count)
  }
...
Function Invoke-ScriptTest {
  ...
  [System.Management.Automation.PSModuleInfo]$Private:PesterModule = Get-LatestAvailableModule -Name 'Pester'
  ...
  [System.Object]$Private:PesterResult = Invoke-Pester -Configuration $PesterConfig
  [System.Double]$Private:CoveragePercent = $PesterResult.CodeCoverage.CoveragePercent

  Test-CodeCoverageGate -CoveragePercent $CoveragePercent -MinimumCoverage $MinimumCoverage
}

Function Invoke-ScriptSmoke {
  [CmdletBinding(
    ConfirmImpact = 'None',
    DefaultParameterSetName = 'default',
    HelpUri = 'https://github.com/NWarila/powershell-template/blob/main/docs/README.md',
...
Function Invoke-ModuleBuild {
  ...
  [System.String[]]$Private:SyntaxTargets = @(
    Get-ChildItem `
      -LiteralPath $SourceRoot `
      -Filter '*.ps1' `
      -File `
  ...
  Test-Syntax -Path $SyntaxTargets
  Write-Information -MessageData 'Module layout: dot-sourced at load; no consolidation.' -InformationAction Continue
}

Function Invoke-ModuleAnalyze {
  [CmdletBinding(
    ConfirmImpact = 'None',
    DefaultParameterSetName = 'default',
    HelpUri = 'https://github.com/NWarila/powershell-template/blob/main/docs/README.md',
  ...
  [System.String]$Private:AnalyzerRuleFile = Join-Path -Path $ProjectRoot -ChildPath 'analyzers\HouseRules.psm1'
  [System.String]$Private:SettingsFile = Join-Path -Path $ProjectRoot -ChildPath 'PSScriptAnalyzerSettings.psd1'
  [System.String]$Private:ManifestPath = Get-ModuleManifestPath
  [System.String]$Private:BuildScriptPath = Join-Path -Path $ProjectRoot -ChildPath 'build.ps1'
  [System.String[]]$Private:SyntaxTargets = @(
    $BuildScriptPath
    $AnalyzerRuleFile
    $ManifestPath
  ...
  [System.Management.Automation.PSModuleInfo]$Private:AnalyzerModule = Get-LatestAvailableModule -Name 'PSScriptAnalyzer'
  If ($Null -eq $AnalyzerModule) {
    Write-Warning -Message 'PSScriptAnalyzer is not installed. Syntax validation passed.'
  } Else {
    Import-Module -Name $AnalyzerModule.Path -Force
    ...
    $private:Results = @(
      Invoke-ScriptAnalyzer -Path $SourceRoot -Recurse -Settings $SettingsFile
      Invoke-ScriptAnalyzer -Path $ManifestPath -Settings $SettingsFile
      Invoke-ScriptAnalyzer -Path $BuildScriptPath -Settings $SettingsFile
      Invoke-ScriptAnalyzer -Path $AnalyzerRuleFile -Settings $SettingsFile
    )
...
Function Invoke-ModuleTest {
  ...
  [System.String]$Private:TestRunner = Join-Path -Path $TestRoot -ChildPath 'Invoke-Tests.ps1'
  [System.Object]$Private:PesterResult = & $TestRunner -MinimumCoverage $MinimumCoverage
  [System.Double]$Private:CoveragePercent = $PesterResult.CodeCoverage.CoveragePercent
...
Function Invoke-ModuleSmoke {
  ...
  Write-Warning -Message 'Module layout has no Smoke directory; skipping smoke.'
}
...
Function Invoke-Test {
  ...
  [System.Double]$Private:EffectiveMinimumCoverage = $Script:RequestedMinimumCoverage
  If ($EffectiveMinimumCoverage -lt 0) {
    $EffectiveMinimumCoverage = If ($Script:Layout -eq 'Script') { 90 } Else { 80 }
  }

  If ($Script:Layout -eq 'Script') {
    Invoke-ScriptTest -MinimumCoverage $EffectiveMinimumCoverage
  } Else {
    Invoke-ModuleTest -MinimumCoverage $EffectiveMinimumCoverage
  }
}
